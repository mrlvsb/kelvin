...
types = {
    "textarea": (forms.CharField, {"widget": forms.Textarea({"rows": 4, "class": "form-control"})}),
    "line": (forms.CharField, {"widget": forms.TextInput({"class": "form-control"})}),
    "integer": (forms.IntegerField, {"widget": forms.NumberInput({"class": "form-control"})}),
    ...
    try:
        ...
        if request.method == "POST" and form.is_valid():
            Answer.objects.update_or_create(
                ...
                defaults={"answers": json.dumps(form.cleaned_data)},
            )
...
def show_csv(request, survey_file):
    ...
    with io.StringIO() as out:
        w = csv.DictWriter(
            out, fieldnames=["login", "submitted"] + [q["name"] for q in conf["questions"]]
        )
        w.writeheader()
