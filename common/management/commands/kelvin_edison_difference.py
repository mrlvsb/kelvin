...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        subject_versions = [subject_version for subject_version in subject_versions if subject_version.subject.abbrev == subject.abbr]
        for subject_version in subject_versions:
            subject_version_schedule = inbus.schedule_subject_by_version_id(subject_version.subjectVersionId)

            # mapping of student -> class
            mapping = {}
            for ca in subject_version_schedule:
                students_inbus = [sr.login.upper() for sr in inbus.students_in_concrete_activity(ca.concreteActivityId)]
                code = ca.code()
                    ...
                    try:
                        ...
                        for k in sorted(students_class):
                            if k not in students_inbus:
                                ...
                                if other_ca:
                                    print(f"\t\tShould be in {other_ca.code()} by {other_ca.teacherLogins}")
