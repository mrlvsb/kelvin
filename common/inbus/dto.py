...
class ConcreteActivity:
    ...
    weekActivities: List[
        WeekActivity
    ]  # seznam výskytů aktivity v konkrétních týdnech výuky (využití při nepravidelné výuce), obsahuje: weekActivityId (Integer), weekNumber (Integer), date (Date)
    roomIds: List[int]  # seznam ID místností
    teacherIds: List[int]  # seznam ID vyučujících (osob)
    studyGroupIds: List[
        int
    ]  # seznam ID studijních skupin (má význam při rozvrhování na studijní skupiny)
