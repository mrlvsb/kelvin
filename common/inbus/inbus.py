...
def person_by_login(login: str) -> dto.PersonSimple | None:
    ...
    person_simple = dto.PersonSimple(
        login=person_json["login"].upper(),
        first_name=person_json.get("firstName", ""),
        second_name=person_json.get("secondName", ""),
        full_name=person_json.get("fullName", ""),
...
def subject_versions(
    department_id: dto.DepartmentId = dto.DepartmentId(386),
) -> List[dto.SubjectVersion]:
    """
    ...
    while True:
        ...
        subject_versions_resp = utils.inbus_request(
            url, {"departmentId": department_id, "offset": offset, "limit": results_per_page}
        )
        ...
        if subject_versions_resp:
            ...
            for subject_version_json in subject_versions_json:
                ...
                subject_version_guarantee = serde.from_dict(
                    dto.Person, subject_version_json["guarantee"]
                )
                subject = dto.Subject(
                    subjectId=subject_json["subjectId"],
                    code=subject_json["code"],
                    abbrev=subject_json["abbrev"],
                    title=subject_json["title"],
                    guarantee=subject_guarantee,
                )
                subject_version = dto.SubjectVersion(
                    subjectVersionId=subject_version_json["subjectVersionId"],
                    subject=subject,
...
def schedule_subject_by_version_id(
    ...
    if concrete_activities_resp:
        ...
        concrete_activities = [
            ...
            for concrete_activity_dict in concrete_activities_json
        ]
...
def concrete_activity(concrete_activity_id: dto.ConcreteActivityId) -> dto.ConcreteActivity | None:
    ...
    if concrete_activity_resp:
        ...
        concrete_activity: dto.ConcreteActivity = serde.from_dict(
            dto.ConcreteActivity, concrete_activity_json
        )
...
def students_in_concrete_activity(
    concrete_activity_id: dto.ConcreteActivityId,
) -> List[dto.StudyRelation]:
    url = urllib.parse.urljoin(
        config.INBUS_SERVICE_EDISON_URL, f"schedule/{concrete_activity_id}/studyRelations"
    )
    ...
    if study_relation_resp:
        ...
        study_relations = [
            serde.from_dict(dto.StudyRelation, study_relation)
            for study_relation in study_relation_json
        ]
