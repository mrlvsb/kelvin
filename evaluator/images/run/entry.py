...
with open("result.html", "w") as f:
    for job in json.loads(os.getenv("PIPE_COMMANDS")):
        ...
        if not job.get("hide", False):
            f.write(
                f"<code style='filter: opacity(.7);'>$ {html.escape(job.get('cmd_show', job['cmd']))}</code><br>"
            )
        ...
        if job.get("asciinema", False):
            ...
            cmd = [
                ...
                "rec",
                "-c",
                f"timeout {job.get('timeout', DEFAULT_TIMEOUT)} {job['cmd']}",
                cast_filepath,
            ]
            ...
            with open(cast_filepath, "rb") as record:
                f.write(
                    f"<asciinema-player preload src='data:application/json;base64,{base64.b64encode(record.read()).decode('utf-8')}'></asciinema-player>"
                )
            ...
            with open("/tmp/out", "w+", errors="ignore") as out:
                ...
                if not job.get("hide", False):
                    out.seek(0)
                    f.write(
                        f"<kelvin-terminal-output>{html.escape(out.read())}</kelvin-terminal-output>"
                    )

                if timedout:
                    f.write(
                        f'<span class="text-danger">Process timed out after {job.get("timeout", DEFAULT_TIMEOUT)} seconds</span>'
                    )
                    exit(1)
