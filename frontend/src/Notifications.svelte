  ...
  <li class="nav-item dropdown">
    <button
      ...
      {#if $notificationsCount > 0}
        <span
          class="badge {$importantNotificationsCount >= 1
            ? 'text-bg-danger'
            : 'text-bg-warning'} border border-light rounded-pill">
          {$notificationsCount}
      ...
      <ul class="list-group list-group-flush">
        <li class="list-group-item">
          <div class="d-flex align-items-center">
            <div>
              Notifications{#if $notificationsCount > 0}&nbsp;({$notificationsCount}){/if}
            </div>
        ...
        {#if $notifications.length > 0}
          {#each $notifications
            .slice()
            .sort((a, b) => ((b.important || 0) && b.unread) - ((a.important || 0) && a.unread) || b.timestamp - a.timestamp) as item (item.id)}
            <li
                ...
                {:else}
                  ...
                  {#if item.action_object_url}
                    <a
                      ...
                      on:auxclick={() => notifications.markRead(item.id)}>
                      {item.action_object}
                    </a>
                  {:else}
                    {item.action_object}
              ...
              <div>
                <button
                  ...
                  class="btn-close"
                  aria-label="Close"
                  on:click={() => notifications.markRead(item.id)}></button>
              </div>
            </li>
          {/each}
        {:else}
