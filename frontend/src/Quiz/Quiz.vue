...
const { quiz_html, is_teacher, student, enrolled_id, answers, remaining, scoring, scored_by } =
  defineProps<{
    quiz_html: string;
    user: number;
    student: string;
...
const answeredData = JSON.parse(answers) as AnswersData;
const scoringData = JSON.parse(scoring) as ScoringData;
...
const remainingTime = parseFloat(remaining) * 1000;
 ...
 */
const sendAnswers = async (answers_type: ResultsType) => {
  if (answers_type == ResultsType.Submit) {
    clearInterval(sendingIntervalHandle);
  }
  const answers = await collectAnswers();
  const data = await getDataWithCSRF<{ redirect?: string }>(
    `/api/quiz/${enrolled_id}/result/${answers_type}`,
    'POST',
    answers
  );
  if (!data) {
...
const collectScoring = async () => {
  ...
  for (const question_id of Object.keys(scoringData)) {
    scoring[question_id] = {
      points: scoringPointsRef.value[question_id].value || 0,
...
onMounted(() => {
  if (mode === Modes.Filling) {
    sendingIntervalHandle = setInterval(async () => {
      await sendAnswers(ResultsType.Intermediate);
    }, SENDING_INTERVAL_MS);
