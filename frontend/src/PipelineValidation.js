...
CodeMirror.registerHelper('hint', 'yaml', function (cm) {
    ...
    try {
        ...
        if (longest !== null) {
            ...
            const isKey =
                list.length &&
                (list[0].constructor == Object
                    ? list[0].text.endsWith(': ')
                    : list[0].endsWith(': '));
...
function yamlSourceMap(lines) {
    ...
    for (let [path, info] of Object.entries(paths)) {
        ...
        path.pop();
        while (path.length) {
            paths[path.join('.')].value.to.line = Math.max(
                paths[path.join('.')].value.to.line,
                info.value.to.line
            );
            path.pop();
        }
    ...
    hint(path, depth, data) {
        if (path == '' || depth >= path.length) {
            ...
            return Object.keys(this.keys)
                .filter((k) => alreadyUsed.indexOf(k) == -1)
                .map((k) => {
                    return {
                        text: k + ': ',
        ...
        if (this.choices.indexOf(data.toString()) < 0) {
            return [
                err(
                    ...
                    `Invalid value. Expected one of: ${this.choices.join(', ')}`
                )
            ];
        }
...
class PipelineRule {
    ...
    hint(path, depth, data) {
        ...
        if (pipeId !== undefined) {
            ...
            if (pipe && pipe['type']) {
                return getRule(this.pipes[pipe['type']]).hint(path, depth + 1, data);
            } else {
                return ['type: '];
            }
        }
...
class DockerPipeRule extends PipeRule {
    constructor(keys) {
        super({
            ...keys,
            before: [
                new ArrayRule(new ValueRule()),
                'List of commands for container preparation. Will be cached for next run.'
            ],
            limits: [
                new DictRule({
                    fsize: [
                        new ValueRule(),
                        'Maximal size for one created file. You can use suffixes like 16M'
                    ],
                    memory: [
                        new ValueRule(),
                        'Maximal memory use for the container, not the process. You can use suffixes like 128M'
                    ],
                    network: [
                        new EnumRule(['none', 'bridge']),
                        '"none" to disable network access (default) or "bridge" to enable it'
                    ]
                }),
...
const rules = new DictRule({
    ...
    timeout: [
        new ValueRule(),
        'Maximal execution of the pipeline in seconds. You can also use <strong>15m</strong> or <strong>1h</strong>.'
    ],
    tests: new ArrayRule(
        new DictRule({
            ...
            exit_code: [
                new ValueRule(),
                'Expected exit code from the process. By default <strong>0</strong>'
            ],
    ...
    pipeline: new PipelineRule({
        ...
        cargo: [
            new DockerPipeRule({
                cmd: [
                    new ValueRule(),
                    'Cargo command that should be executed. Default is `build`.'
                ],
                args: [new ArrayRule(new ValueRule()), 'Arguments for Cargo.'],
                lib: [
                    new ValueRule(),
                    'Whether the submitted file should be compiled as a library.'
                ]
            }),
                    ...
                    new ArrayRule(
                        new UnionRule(
                            new DictRule({
                                ...
                                display: [
                                    new ArrayRule(new ValueRule()),
                                    'List of image/video patterns that will be converted and shown on the result page.'
                                ],
                                hide: [
                                    new EnumRule(['true', 'false']),
                                    'Hide command and its output from the result page.'
                                ],
                                asciinema: [
                                    new EnumRule(['true', 'false']),
                                    'Run the command in asciinema and generate video animation from the run.'
                                ],
                                timeout: [
                                    new ValueRule(),
                                    'Timeout in seconds. You can also suffixes like 5m'
                                ]
                            }),
                            new ValueRule()
                        )
        ...
        flake8: [
            new DockerPipeRule({
                select: [
                    new ArrayRule(new ValueRule()),
                    'List of enabled PEP8 codes. Can be array or string delimited by comma'
                ],
                ignore: [
                    new ArrayRule(new ValueRule()),
                    'List of ignored PEP8 codes. Can be array or string delimited by comma'
                ]
            }),
        ...
        'clang-tidy': [
            new DockerPipeRule({
                checks: [
                    new ArrayRule(new ValueRule()),
                    'List of used <a href="https://clang.llvm.org/extra/clang-tidy/checks/list.html">checks</a>. You may use asterisks <strong>*</strong> or block checks with hyphen <strong>-</strong>.'
                ],
        ...
        auto_grader: [
            new PipeRule({
                propose: [
                    new EnumRule(['true', 'false']),
                    'Only propose points without assigning.'
                ],
                overwrite: [
                    new EnumRule(['true', 'false']),
                    'Do not overwrite already assigned points in current submit.'
                ],
                after_deadline_multiplier: [
                    new ValueRule(),
                    'Points multiplier in range of 0 to 1 when submit is after deadline'
                ]
            }),
