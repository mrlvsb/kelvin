<script>
    ...
    function target_changed() {
        ...
        let index = 0;
        assignments = classes.find(c => c.code == clazz).assignments.map(t => {
            return {
                'name': t.name,
...
{#if result}
    ...
    {:else}
    <button class="btn btn-success" on:click={transfer} disabled={!canTransfer} class:btn-danger={!canTransfer}>
        {#if busy}
        Transfering...
        {:else}
            Transfer
