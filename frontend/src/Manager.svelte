  ...
  <div class="tree">
    ...
    <ul>
      ...
      {#each $cwd as inode (inode)}
        <li
          on:contextmenu|preventDefault={(e) => showCtxMenu(e, inode.name)}
      ...
      {#each Object.entries($openedFiles).filter(([_, inode]) => !inode.hide_tab) as [path, file]}
        <li class="nav-item" style="cursor: pointer">
          <span class="nav-link" class:active={path === $currentOpenedFile}>
            ...
            <span on:click={() => fs.close(path)}
              ><span class="iconify" data-icon="fa:times"></span></span>
          </span>
        </li>
      {/each}
    </ul>
    ...
    <div class="editor-container">
      ...
      {#if $currentOpenedFile}
        <Editor
          ...
          bind:value={$openedFiles[$currentOpenedFile].content} />
      {/if}
    </div>
  </div>
</div>
...
{#if ctxMenu && ctxMenu.selected != '/readme.md'}
  <div
    ...
    use:clickOutside
    on:click_outside={() => (ctxMenu = null)}>
    <button
      class="dropdown-item"
      on:click|preventDefault={() => {
        renamingPath = ctxMenu.selected;
        ctxMenu = null;
      }}><span class="iconify" data-icon="wpf:rename"></span> rename</button>
    <button
      class="dropdown-item"
      on:click|preventDefault={() => {
        remove(ctxMenu.selected);
        ctxMenu = null;
      }}><span class="iconify" data-icon="wpf:delete"></span> delete</button>
  </div>
{/if}
