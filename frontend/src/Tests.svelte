    ...
    {#each Object.values(tests) as test}
      <h2>
        ...
        {#each ['stdin', 'stdout', 'stderr'] as fd}
          {#if !test.files[fd]}
            <button
              on:click={add_file(test.name, fd, fd.replace('std', ''))}
        ...
        <div class="btn-group" role="group" aria-label="Basic example">
          ...
          <button
            ...
            on:click={() =>
              test.new_filename &&
              add_file(test.name, test.new_filename, `file_in.${test.new_filename}`)}
            ><span class="iconify" data-icon="ant-design:plus-outlined"></span>input</button>
          <button
            type="button"
            class="btn btn-sm btn-success text-nowrap"
            on:click={() =>
              test.new_filename &&
              add_file(test.name, test.new_filename, `file_out.${test.new_filename}`)}
            ><span class="iconify" data-icon="ant-design:plus-outlined"></span>output</button>
        </div>
