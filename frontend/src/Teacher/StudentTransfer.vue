...
const sortedSemesters = computed(() => {
  return Object.keys(data.semesters).sort((a, b) => {
    const [yearA, termA] = [a.slice(0, 4), a.slice(4)];
    const [yearB, termB] = [b.slice(0, 4), b.slice(4)];

    if (yearA !== yearB) {
      return parseInt(yearB) - parseInt(yearA); // descending year
    }

    // Define custom order: S (summer) < W (winter/fall)
    const termOrder = { S: 0, W: 1 };
...
<template>
  <div v-if="target_class" class="container mb-4">
    <b>Tasks on the right side need to be unique!</b>
  </div>
  <div v-if="data">
    <select v-model="semester">
      <option v-for="item in sortedSemesters" :key="item" :value="item">
        {{ item }}
      </option>
    </select>
    ...
    <select v-if="student" v-model="target_class" @change="target_changed">
      <option
        v-for="item in classes.filter((c) => c.code !== clazz)"
        :key="item.code"
        :value="item.code"
      >
